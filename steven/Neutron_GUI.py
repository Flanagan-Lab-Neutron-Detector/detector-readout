...
def get_voltage_sweep(port,base_address,voltages):
    values = []
    tracker = 0
...
tab2_layout = [
    ...
    [sg.Button('Sweep Voltages', key = '-SWEEP SUBMIT-'),sg.Button('Bit Noise', key = '-BIT NOISE-')]
]
...
while True:
    ...
    if event == '-PORTS-':
        ...
        try:
            ser = serial.Serial(values['-PORTS-'][0], 115200, bytesize = serial.EIGHTBITS,stopbits =serial.STOPBITS_ONE, parity  = serial.PARITY_NONE,timeout=1)
        ...
        resp_values = [values['-SECTOR ADDRESS-'],values['-START MV-'],values['-END MV-'],values['-STEP MV-']]
        try:
            resp_values = [int(x) for x in resp_values]
        except e:
            print(e)
            continue
        get_voltage_sweep(ser, resp_values[0],range(resp_values[1],resp_values[2],resp_values[3]))
