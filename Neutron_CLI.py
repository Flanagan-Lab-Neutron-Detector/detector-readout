...
#import nisoc_readout as comms
import nisoc_readout_dummy as comms
...
def read_and_save_chip_voltages(port, voltages, location='.', identifier=None):
    if identifier is None:
        identifier = time.time()
    printProgressBar(0, len(voltages)*128*1024, prefix='Getting sweep data: ', suffix='complete', decimals=0, length=50)
    for sector_index, sector_address in enumerate(range(0, 0x4000000, 0xFE00 + 0x200)):
        for v_index, voltage in enumerate(voltages):
            f = open(os.path.join(f"{location}", f"{identifier}-{voltage}mv-SA{sector_address}.bin"), "ab")
            for idx, address in enumerate(range(sector_address, sector_address + 0x10000, 0x200)):
                data_chunk = comms.read_data(port, address, True, voltage)
                f.write(data_chunk)
            f.close()
                #time.sleep(0.003)
...
parser.add_argument('-lp', '--list-ports', action='store_true', help='list COM port options')

parser.add_argument('-p', '--port', help='the COM port where the chip reader is installed')

parser.add_argument('-r',  '--read', action='store_true', help='read data from the chip')
parser.add_argument('--sector', type=auto_int, help='the sector of the chip to read')
parser.add_argument('--start', type=auto_int, help='the lowest voltage at which to read the chip in mV')
parser.add_argument('--stop', type=auto_int, help='the highest voltage at which to read the chip in mV')
parser.add_argument('--step', type=auto_int, help='the granularity in mV')
...
# if args.read and (not args.all_sectors) and ((args.sector is None) or (args.start is None) or (args.stop is None) or (args.step is None)):
...
if args.read and (not ((args.sector is None) or args.all_sectors) or (args.start is None) or (args.stop is None) or (args.step is None)):
    parser.error("--read requires --sector or all-sectors, --start, --stop, and --step \n"
                 "suggested test values are --sector 800000 --start 2000 --stop 7000 --step 1000")
...
if(args.port):
    ...
    uptime, fw_version, is_busy = comms.ping(ser)
        
    print(f"Connected to board. FW version = {fw_version}")
    print(f"status idle={bool(is_busy)}")
...
# Read a sector or entire chip
    ...
    if(args.sector):
        ...
        read_and_save_chip_voltages(ser, range(args.start, args.stop, args.step), location=directory)
        #read_chip_voltages(ser, range(args.start, args.stop, args.step), location=directory)
        
# Erase a sector or entire chip
elif(args.erase):
    if(args.sector):
        comms.erase_sector(ser, args.sector)
    elif(args.all_sectors):
        comms.erase_chip(ser)

# Write a sector or entire chip
elif(args.write):
    if(args.sector):
        comms.program_sector(ser, args.sector, args.value)
    elif(args.all_sectors):
        comms.program_chip(ser, args.value)
        print("Writing a chip takes about 30 minutes and will continue despite the serial error that causes this macro to abort. Please wait until the status LED stops flashing.")
